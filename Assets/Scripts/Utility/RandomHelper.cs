...
namespace ET
{
    ...
    public static class RandomHelper
    {
        ...
        {
            int value = random.Next((int)Math.Floor(lower*100), (int)Math.Floor(upper*100));
            return value/100f;
        }
        ...
        {
            sum = Math.Min(sum, Math.Abs(max - min) + 1);
            int[] arr = new int[sum];
            int j = 0;
            //表示键和值对的集合。
        ...
        /// <returns></returns>
        public static bool GetRandomTrigger(int rate)
        {
